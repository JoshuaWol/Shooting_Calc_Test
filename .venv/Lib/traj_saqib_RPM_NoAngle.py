...
def calc_distances() -> list[tuple[float, float, float]]:
    ...
    # max_vi_ten = int(max(vi_values)*10)
    # min_vi_ten = int(min(vi_values)*10)
    # i = 0
    # data_set = []
    # for RPM in range(2600, 4850, 50):
    #     angle = angle_values
...
with open(os.path.join(directory, f"trajectories_Var_RPM_NoHood.json"),"w") as file: #_3800RPM_dist_theta_vi.json"), "w") as file:
