...
def calc_distances() -> list[tuple[float, float, float]]:
    servo_values = [0.2, 0.4, 0.62, 0.8, 1.0]
    angle_values = [73.2, 68.8, 63.9, 59.9, 55.5]
    vi_values = [9.3, 9.3, 9.3, 9.3, 9.3]
    max_angle_ten = int(max(angle_values)*10)
    min_angle_ten = int(min(angle_values)*10)
    i = 0
    data_set = []
    for angle_ten in range(-max_angle_ten,-min_angle_ten+1, 1):
        angle = -angle_ten / 10
        if angle > angle_values[i]:
            i += 1
        if angle == angle_values[i]:
            servo_set = servo_values[i]
            vi_set = vi_values[i]
        else:
            m_servo = (servo_values[i] - servo_values[i - 1]) / (angle_values[i] - angle_values[i - 1])
            servo_set = m_servo * (angle - angle_values[i - 1]) + servo_values[i - 1]

            m_vi = (vi_values[i] - vi_values[i - 1]) / (servo_values[i] - servo_values[i - 1])
            vi_set = m_vi * (servo_set - servo_values[i - 1]) + vi_values[i - 1]
        data_set.append((angle, vi_set, servo_set))
    print(len(data_set))
    ...
    for each in data_set:
        ...
        # if math.isclose(z, target_z, rel_tol=1) and prev_z < target_z and z > target_z:
            ...
            result.append((x, math.degrees(theta_i), vi, servo_i))

        xmax = max(x,xmax)
        if xmax == x:
            best = (vi,math.degrees(theta_i),x, servo_i)
...
with open(os.path.join(directory, f"trajectories_{RPM}RPM.json"),"w") as file: #_3800RPM_dist_theta_vi.json"), "w") as file:
